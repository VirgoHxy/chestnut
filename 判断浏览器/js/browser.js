var getOSAndBrowser = function () {
    ...
    var is360 = 'track' in document.createElement('track');
    ...
    if (/[Ff]irefox(\/\d+\.\d+)/.test(userAgent)) {
        ...
        if (_testExternal(/^sogou/i, 0)) {// 搜狗浏览器
            info += "sogou";
        } else if (_testExternal(/^liebao/i, 0)) {// 猎豹浏览器
            info += "liebao";
        } else if (_mime("type", "application/vnd.chromium.remoting-viewer")) {// 360浏览器
            info += "360";
        } else {
        	info += tempArray[1] + tempArray[2];
        }
